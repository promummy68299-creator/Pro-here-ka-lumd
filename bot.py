...
# ==================== CONFIG ====================
BOT_TOKEN = "8579503407:AAEFs4y4AKJY5N6RzwZShklHXMaNmZC8It4"
ADMIN_IDS = []
    ...
    def __init__(self):
        ...
        if os.path.exists(self.db_path):
            try:
                ...
                logger.warning(f"Corrupt database detected: {e}. Deleting...")
                os.remove(self.db_path)
                logger.info("🗑️ Corrupt database deleted, creating new one...")
        self.init_tables()
...
def admin_management_keyboard():
    ...
    kb.row(
        ...
        types.InlineKeyboardButton("❌ Remove Admin", callback_data="admin_remove_admin_list")
    )
    kb.add(types.InlineKeyboardButton("📋 View Admins", callback_data="admin_view_admins"))
    kb.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel"))
    return kb
...
def admin_cmd(msg):
    ...
    if is_admin(user_id):
        ...
        sent = safe_send(user_id, text, reply_markup=admin_keyboard(user_id))
        if sent:
            user_data[user_id] = {'last_msg_id': sent.message_id}
    else:
    ...
    try:
        if data == "back_main":
            ...
            if plan:
                ...
                if not upi:
                    safe_send(user_id, "❌ UPI ID not configured by admin. Please contact support.")
                    return
                amount = int(plan['price'])
                qr_img = generate_qr(upi, amount, BOT_NAME)
                if qr_img:
                    try:
                        sent_msg = bot.send_photo(user_id, qr_img, caption=f"💳 <b>Scan & Pay Securely</b> ✨\n"
                        "━━━━━━━━━━━━━━\n"
                        f"📦 Plan: {plan['name']}\n"
                        f"💰 Amount: ₹{amount}\n"
                        ...
                        "✅ The amount is filled automatically.\n"
                        "📸 After paying, tap Verify Payment Status and send screenshot.",
                        reply_markup=payment_keyboard(plan_id))
                        if sent_msg and sent_msg.photo:
                            QR_CODE = sent_msg.photo[-1].file_id
                            db.set_setting('qr_code', QR_CODE)
                    except Exception as e:
                        logger.error(f"QR send error: {e}")
                        safe_send(user_id, "❌ Failed to generate QR. Please try again.")
                else:
                    safe_send(user_id, "❌ Failed to generate QR. Please check UPI ID.")
                bot.delete_message(call.message.chat.id, call.message.message_id)
            else:
                bot.answer_callback_query(call.id, "Plan not found!")
            ...
            if is_admin(user_id):
                load_admins()
                text = f"<b>⚙️ Admin Panel</b>\n\nWelcome {BOT_NAME} admin!"
                safe_edit(call.message.chat.id, call.message.message_id, text, reply_markup=admin_keyboard(user_id))
            else:
            ...
            if is_admin(user_id):
                ...
                safe_edit(call.message.chat.id, call.message.message_id, text, reply_markup=admin_plans_keyboard())
            ...
            if is_admin(user_id):
                ...
                safe_edit(call.message.chat.id, call.message.message_id,
                         "📋 <b>Plan Management</b>", reply_markup=admin_plans_keyboard())
            ...
            if is_admin(user_id):
                try:
                    with open(DATABASE_PATH, 'rb') as f:
                        bot.send_document(user_id, f, caption="📤 Database exported successfully!")
                except Exception as e:
            ...
            if is_admin(user_id):
                ...
                safe_edit(call.message.chat.id, call.message.message_id,
                         "📥 Send the `.sql` file to import.\n⚠️ This will execute SQL queries on current database!",
                         reply_markup=types.InlineKeyboardMarkup().add(
                         types.InlineKeyboardButton("🔙 Cancel", callback_data="admin_panel")))
                ...
                if payment:
                    ...
                    if user and plan:
                        ...
                        if link:
                            ...
                            bot.send_message(payment['user_id'], text, disable_web_page_preview=True)
            ...
            if is_admin(user_id) and ADMIN_IDS and user_id == ADMIN_IDS[0]:
                text = "👑 <b>Admin Management</b>\n\nManage your bot admins here.\nOnly main admin can manage admins."
                safe_edit(call.message.chat.id, call.message.message_id, text, reply_markup=admin_management_keyboard())
            else:
            ...
            if is_admin(user_id) and ADMIN_IDS and user_id == ADMIN_IDS[0]:
                ...
                safe_edit(call.message.chat.id, call.message.message_id,
                         ...
                         reply_markup=types.InlineKeyboardMarkup().add(
                         types.InlineKeyboardButton("🔙 Cancel", callback_data="admin_management")))
            else:
                bot.answer_callback_query(call.id, "❌ Unauthorized!")
            ...
            if is_admin(user_id) and ADMIN_IDS and user_id == ADMIN_IDS[0]:
                ...
                for i, admin in enumerate(admins, 1):
                    name = admin[1] or 'Unknown'
                    uname = admin[2] or ''
                    uid = admin[0]
                    is_main = "⭐ MAIN" if i == 1 else ""
                    text += f"{i}. {name} {is_main}\n"
                    text += f"   🆔 {uid}\n"
                    if uname:
                        text += f"   @{uname}\n"
                    text += "\n"
                safe_edit(call.message.chat.id, call.message.message_id, text,
                         reply_markup=types.InlineKeyboardMarkup().add(
                         types.InlineKeyboardButton("🔙 Back", callback_data="admin_management")))
            else:
                bot.answer_callback_query(call.id, "❌ Unauthorized!")
            ...
            if is_admin(user_id) and ADMIN_IDS and user_id == ADMIN_IDS[0]:
                ...
                for admin in admins:
                    if admin[0] != user_id:
                        name = admin[1] or 'Unknown'
                        kb.add(types.InlineKeyboardButton(f"❌ Remove {name}", 
                                 callback_data=f"admin_remove_{admin[0]}"))
...
def handle_document(msg):
    global db  # <-- FIX: global declaration at the very top
    user_id = msg.from_user.id
    if user_id in user_data and user_data[user_id].get('import_db'):
        file_id = msg.document.file_id
        file_name = msg.document.file_name.lower()
        if not file_name.endswith('.sql'):
            bot.reply_to(msg, "❌ Please send a `.sql` file only!")
            return
        try:
            file_info = bot.get_file(file_id)
            downloaded_file = bot.download_file(file_info.file_path)
            sql_content = downloaded_file.decode('utf-8')
            conn = db.get_conn()
            c = conn.cursor()
            ...
            for query in queries:
                ...
                if query:
                    try:
                        ...
                        errors.append(f"❌ {str(e)[:50]}...")
            conn.commit()
            conn.close()
            if errors:
                bot.reply_to(msg, f"⚠️ SQL imported with {executed} successful, {len(errors)} errors:\n" + "\n".join(errors[:5]))
            else:
...
def handle_text(msg):
    ...
    if user_id in user_data and user_data[user_id].get('add_admin'):
        try:
            ...
            if not user:
                bot.reply_to(msg, "❌ User not found! Make sure they have started the bot first.")
                del user_data[user_id]
                return
            ...
            load_admins()
            bot.reply_to(msg, f"✅ User {new_admin_id} is now an admin!")
            try:
                bot.send_message(new_admin_id, "👑 <b>You are now an admin!</b>\n\nUse /admin to access the admin panel.", parse_mode='HTML')
            except:
                pass
            del user_data[user_id]
        except ValueError:
    ...
    if user_id in user_data and 'setting' in user_data[user_id]:
        ...
        if key == 'welcome_text':
            ...
            global UPI_ID
            UPI_ID = msg.text
            qr_img = generate_qr(msg.text, 0, BOT_NAME)
            if qr_img:
                try:
                    sent = bot.send_photo(user_id, qr_img, caption="✅ UPI ID updated! QR code auto-generated.")
                    if sent and sent.photo:
                        db.set_setting('qr_code', sent.photo[-1].file_id)
                        global QR_CODE
                        QR_CODE = sent.photo[-1].file_id
                except:
                    bot.reply_to(msg, "✅ UPI ID updated! QR generation failed, but UPI is saved.")
            else:
                bot.reply_to(msg, "✅ UPI ID updated! QR generation failed.")
        del user_data[user_id]
        return
    if user_id in user_data and user_data[user_id].get('broadcast'):
        users = db.get_all_users()
