...
BOT_TOKEN = "8704774110:AAGrzMBAPyghRUafl40hH408rqWlreSQux4"  # <-- Apna token daalo
ADMIN_IDS = [8279891640, 7709767483]  # <-- Apni admin IDs daalo (comma separated)
DATABASE_PATH = 'bot_database.db'
PORT = int(os.getenv('PORT', 8080))

# Paytm Config (Hardcoded)
PAYTM_MERCHANT_ID = "YOUR_MERCHANT_ID"  # <-- Paytm Merchant ID
PAYTM_MERCHANT_KEY = "YOUR_MERCHANT_KEY"  # <-- Paytm Merchant Key
PAYTM_MERCHANT_WEBSITE = "WEBSTAGING"
PAYTM_INDUSTRY_TYPE = "Retail"
PAYTM_CHANNEL_ID = "WEB"
PAYTM_ENV = "TEST"  # TEST or PROD
PAYTM_CALLBACK_URL = "https://your-bot.railway.app/paytm-callback"  # <-- Callback URL
...
class Database:
    ...
    def init_tables(self):
        ...
        defaults = [
            ...
            ('paytm_callback_url', PAYTM_CALLBACK_URL)
        ]
...
def generate_upi_link(upi_id, amount, name=''):
    if not upi_id:
        return ''
    ...
    if '@' in upi_id:
        return f"upi://pay?pa={quote(upi_id)}&pn={quote(name)}&am={amount}&cu=INR&tn=Premium%20Subscription%20Payment"
    else:
        return f"upi://pay?pa={quote(upi_id)}@paytm&pn={quote(name)}&am={amount}&cu=INR&tn=Premium%20Subscription%20Payment"
...
class HealthHandler(BaseHTTPRequestHandler):
    ...
    def handle_callback(self):
        try:
            merchant_key = db.get_setting('paytm_merchant_key') or PAYTM_MERCHANT_KEY
...
def plan_detail_keyboard(plan_id):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("💳 Auto Pay (Paytm)", callback_data=f"paytm_pay_{plan_id}"))
    kb.add(types.InlineKeyboardButton("💰 Manual Payment (UPI)", callback_data=f"manual_pay_{plan_id}"))
...
def payment_settings_keyboard():
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("🔄 Auto Payment (Paytm)", callback_data="ps_auto_toggle"),
...
def payment_settings_cmd(msg):
    user_id = msg.from_user.id
    if is_admin(user_id):
        text = f"<b>💳 Payment Settings</b>\n\nConfigure payment methods:"
        safe_send(user_id, text, reply_markup=payment_settings_keyboard())
    else:
        safe_send(user_id, "❌ Unauthorized access!")
...
def handle_cb(call):
    ...
    try:
        ...
        # ========== PAYTM PAYMENT ==========
            ...
            merchant_id = db.get_setting('paytm_merchant_id') or PAYTM_MERCHANT_ID
            merchant_key = db.get_setting('paytm_merchant_key') or PAYTM_MERCHANT_KEY
            callback_url = db.get_setting('paytm_callback_url') or PAYTM_CALLBACK_URL
        ...
        # ========== MANUAL PAYMENT ==========
            ...
            upi_link = generate_upi_link(upi, plan['price'], merchant_name)
