...
ADMIN_IDS = [7924753922, 7709767483]
...
PAYMENT_MODE = db.get_setting('payment_mode')
            ...
            if txn_status == 'TXN_SUCCESS':
                ...
                if db.activate_premium_from_paytm(order_id):
                    ...
                    if order:
                        ...
                        if user and plan:
                            link = db.get_setting('delivery_link') or ''
                            ...
                            if link:
                                text += f"🔗 <a href='{link}'>Access Premium Content</a>"
...
def plan_detail_keyboard(plan_id):
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(types.InlineKeyboardButton("✅ I HAVE PAID", callback_data=f"pay_now_{plan_id}"))
...
def admin_keyboard():
    ...
    kb.row(
        ...
        types.InlineKeyboardButton("💳 Payment Mode", callback_data="admin_payment_mode")
    )
    kb.row(
...
def payment_settings_keyboard():
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("🔐 Merchant Secret", callback_data="ps_secret")
    )
    kb.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel"))
    return kb

def payment_mode_keyboard():
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.row(
        types.InlineKeyboardButton("💰 Manual Payment", callback_data="pm_manual"),
        types.InlineKeyboardButton("🤖 Auto Payment", callback_data="pm_auto")
    )
    kb.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_panel"))
    return kb
    ...
    try:
        ...
        # ========== VIEW PLAN ==========
            ...
            if plan:
                ...
                # Direct payment flow based on mode
                process_payment(user_id, plan_id)
            else:
        ...
        # ========== ADMIN SETTINGS ==========
        ...
        elif data == "admin_payment_mode":
            if is_admin(user_id):
                current = db.get_setting('payment_mode') or 'manual'
                text = f"<b>💳 Payment Mode</b>\n\n"
                text += f"Current Mode: <b>{current.upper()}</b>\n\n"
                text += "Select payment mode:"
                safe_edit(call.message.chat.id, call.message.message_id, text,
                         reply_markup=payment_mode_keyboard())
        
        elif data == "pm_manual":
            if is_admin(user_id):
                db.set_setting('payment_mode', 'manual')
                PAYMENT_MODE = 'manual'
                bot.answer_callback_query(call.id, "✅ Manual Payment Mode Enabled!")
            ...
            if is_admin(user_id):
                db.set_setting('payment_mode', 'auto')
                PAYMENT_MODE = 'auto'
                bot.answer_callback_query(call.id, "✅ Auto Payment Mode Enabled!")
        ...
        # ========== PAYMENT SETTINGS ==========
            ...
            if is_admin(user_id):
                ...
                secret = '****' if db.get_setting('paytm_merchant_secret') else 'Not set'
            ...
            if is_admin(user_id):
                user_data[user_id] = {'payment_setting': 'paytm_merchant_secret'}
                safe_edit(call.message.chat.id, call.message.message_id,
                         "🔐 Send Paytm Merchant Secret:\n\nKeep this secret!",
                         reply_markup=types.InlineKeyboardMarkup().add(
                         types.InlineKeyboardButton("🔙 Cancel", callback_data="admin_payment_settings")))
                ...
                if db.approve_payment(pid):
                    ...
                    if payment:
                        ...
                        if user and plan:
                            ...
                            text += f"📅 Validity: {plan['validity_days']} days\n"
                            if link:
                                text += f"\n🔗 <a href='{link}'>Access Premium Content</a>"
...
def process_payment(user_id, plan_id):
    plan = db.get_plan(plan_id)
    if not plan:
        bot.send_message(user_id, "❌ Plan not found!")
        return
    
    payment_mode = db.get_setting('payment_mode') or 'manual'
    ...
    if payment_mode == 'manual':
        ...
        if not upi:
            bot.send_message(user_id, "❌ UPI not configured! Contact admin.")
            return
        
        order_id = generate_order_id()
        user_data[user_id] = {'manual_plan': plan_id, 'order_id': order_id}
        
        # Generate UPI link with amount from plan
        ...
        bot.send_message(user_id, "After payment, click <b>I HAVE PAID</b>", reply_markup=kb)
    
    else:
        # Auto Payment Flow (Paytm)
        merchant_id = db.get_setting('paytm_merchant_id')
        merchant_key = db.get_setting('paytm_merchant_key')
        
        if not merchant_id or not merchant_key:
            bot.send_message(user_id, "❌ Auto payment not configured! Contact admin.")
            return
        
        order_id = generate_order_id()
        user_data[user_id] = {'auto_plan': plan_id, 'order_id': order_id}
        
        if not db.create_paytm_order(order_id, user_id, plan_id, plan['price']):
            bot.send_message(user_id, "❌ Failed to create order!")
            return
        ...
        bot.send_message(user_id, "🔄 Redirecting to Paytm...")
...
def handle_photo(msg):
    ...
    # Screenshot upload for manual payment
            ...
            if payment:
                ...
                for admin in ADMIN_IDS:
                    try:
                        ...
                        logger.info(f"Manual payment notification sent to admin {admin}")
                    except Exception as e:
...
def handle_text(msg):
    ...
    # ===== I HAVE PAID - Manual Payment =====
    if user_id in user_data and 'manual_plan' in user_data[user_id]:
        plan_id = user_data[user_id]['manual_plan']
        plan = db.get_plan(plan_id)
        if plan:
            ...
            del user_data[user_id]['manual_plan']
        return
