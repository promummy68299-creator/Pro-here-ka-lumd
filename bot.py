...
# ==================== CONFIG ====================
BOT_TOKEN = "8912326354:AAEk-Eki3qX-ngifx9OtTiciGDelRTBfiN4"  # Apna token daalo
ADMIN_IDS = []
...
def start_cmd(msg):
    ...
    # ===== WELCOME VIDEO/IMAGE + TEXT EK SATH =====
    if WELCOME_VIDEO:
    ...
    # ===== CHOOSE PLAN BELOW (ALAG MESSAGE) =====
    ...
    try:
        ...
        # ========== ADMIN PANEL ==========
            ...
            if is_admin(user_id):
                ...
                text += f"💳 Paytm Success: {s['paytm_success']}"
                safe_edit(call.message.chat.id, call.message.message_id, text,
        ...
        # REMOVED: admin_qr callback (QR Upload removed)
        ...
        # ========== APPROVE PAYMENT ==========
            ...
            try:
                ...
                success = db.approve_payment(pid)
...
def handle_photo(msg):
    ...
    if user_id in user_data and 'setting' in user_data[user_id]:
        ...
        # REMOVED: QR Code upload (key == 'qr_code')
...
def handle_document(msg):
    ...
    if user_id in user_data and user_data[user_id].get('import_db'):
        try:
            processing_msg = bot.reply_to(msg, "⏳ Importing database... Please wait.")
            ...
            logger.error(f"Import error: {e}")
            bot.reply_to(msg, f"❌ Import failed! Error: {str(e)}")
            del user_data[user_id]
        return
...
def handle_text(msg):
    ...
    if user_id in user_data and 'reject_payment' in user_data[user_id]:
        ...
        success = db.reject_payment(pid, reason)
        
        if not success:
            bot.reply_to(msg, f"❌ Failed to reject payment!")
            del user_data[user_id]
            return
